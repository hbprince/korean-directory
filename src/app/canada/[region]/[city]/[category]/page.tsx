...
export async function generateStaticParams() {
  ...
  console.log(`[generateStaticParams] Canada pages: ${params.length} paths`);
  return params;
}
...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  ...
  return generateIntlMetadata('canada', region, city, category, page);
}

export default async function CanadaCategoryPage({ params, searchParams }: PageProps) {
  const { region, city, category } = await params;
  const { page: pageParam } = await searchParams;
